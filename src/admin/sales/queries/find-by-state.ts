...
const findByStateSchema = z.object({
  edition: z.string(),
...
type FindByStateParams = z.infer<typeof findByStateSchema>;
...
export const findByState = ({
  ...
  if (param) {
    return Prisma.sql`
      SELECT buyed_titles.address_state, 
      ...
      WHERE buyed_titles.address_state = ${param} AND editions.name = ${edition}
      GROUP BY buyed_titles.address_state
      LIMIT ${limit}
      OFFSET ${limit * offset};
    `;
  }
...
export const findByStateCount = (param: string, edition: string) => {
  if (param) {
    return Prisma.sql`
